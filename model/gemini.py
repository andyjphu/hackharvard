...
class BrowserScreenshotAnalyzer:
    ...
    def capture_screenshot(self, monitor: int = 1) -> str:
        """
        ...
        try:
            with mss.mss() as sct:
                ...
                print(f"✅ Screenshot captured and saved to: {self.screenshot_path}")
    ...
    def analyze_screenshot(self, image_path: str, user_task: str = "") -> Dict[str, Any]:
        """
    ...
    def _highlight_task_relevant(self, elements: List[Dict], user_task: str) -> List[Dict]:
        ...
        for element in elements:
            ...
            if "search" in task_lower and element_type in ['input', 'search']:
                is_relevant = True
            elif "login" in task_lower and ('login' in element_text or 'sign' in element_text or 'auth' in element_purpose):
                is_relevant = True
            elif "form" in task_lower and element_type in ['input', 'textarea', 'select']:
                is_relevant = True
            elif "button" in task_lower and element_type == 'button':
                is_relevant = True
            elif "submit" in task_lower and ('submit' in element_text or 'submit' in element_purpose):
                is_relevant = True
    ...
    def display_analysis_results(self, analysis: Dict[str, Any], user_task: str = ""):
        ...
        if elements:
            ...
            print(self._draw_border(f"🔘 INTERACTIVE ELEMENTS FOUND ({len(elements)})", "─", 50))
            
            # Display task-relevant elements first with special highlighting
            relevant_elements = [e for e in sorted_elements if e.get('task_relevant', False)]
            other_elements = [e for e in sorted_elements if not e.get('task_relevant', False)]
            ...
            if relevant_elements:
                ...
                for i, element in enumerate(relevant_elements, 1):
                    ...
                    print(f"│ {i}. {element.get('type', 'Unknown').upper():<12} │ {element.get('text', 'No text')[:35]:<35} │")
                    print(f"│    Position: {element.get('position', 'Unknown'):<43} │")
                    print(f"│    Purpose:  {element.get('purpose', 'Unknown')[:43]:<43} │")
                    print(f"│    Visual:   {element.get('characteristics', 'None')[:43]:<43} │")
            ...
            if other_elements:
                ...
                for i, element in enumerate(other_elements, 1):
                    ...
                    print(f"      Characteristics: {element.get('characteristics', 'None')}")
                    print()
        else:
        ...
        if warnings:
            ...
            for warning in warnings:
                # Wrap long warnings
                wrapped_warning = [warning[i:i+56] for i in range(0, len(warning), 56)]
    ...
    def generate_step_by_step_instructions(self, analysis: Dict[str, Any], user_task: str) -> List[str]:
        ...
        if not elements:
            instructions.append("No interactive elements found to provide specific instructions.")
            return instructions
        ...
        if "search" in task_lower:
            search_elements = [e for e in elements if e.get('type') in ['input', 'search']]
            if search_elements:
                element = search_elements[0]
                coords = element.get('coordinates', {})
                click_x = coords.get('click_x', 0)
                click_y = coords.get('click_y', 0)
                instructions.append(f"1. Locate the search bar at position ({click_x}, {click_y})")
                instructions.append(f"2. Click at coordinates ({click_x}, {click_y}) to activate the search bar")
                instructions.append("3. Type your search query")
                instructions.append("4. Press Enter or click the search button")
            else:
                instructions.append("1. No search bar found - look for a search icon or menu option")
        
        elif "login" in task_lower or "sign in" in task_lower:
            login_elements = [e for e in elements if 'login' in e.get('text', '').lower() or 'sign' in e.get('text', '').lower()]
            if login_elements:
                element = login_elements[0]
                coords = element.get('coordinates', {})
                click_x = coords.get('click_x', 0)
                click_y = coords.get('click_y', 0)
                instructions.append(f"1. Click the login/sign-in button at coordinates ({click_x}, {click_y})")
                instructions.append("2. Enter your username/email in the username field")
            ...
            form_elements = [e for e in elements if e.get('type') in ['input', 'textarea', 'select']]
            if form_elements:
                ...
                for i, element in enumerate(form_elements[:5], 2):  # Limit to 5 fields
                    ...
                    click_x = coords.get('click_x', 0)
                    click_y = coords.get('click_y', 0)
                    instructions.append(f"{i}. Click at ({click_x}, {click_y}) and fill in the {element.get('text', 'form field')}")
                instructions.append(f"{len(form_elements) + 2}. Review all information before submitting")
                instructions.append(f"{len(form_elements) + 3}. Click the submit button")
            else:
            ...
            if relevant_elements:
                ...
                click_x = coords.get('click_x', 0)
                click_y = coords.get('click_y', 0)
                instructions.append(f"1. Click on the {element.get('type', 'element')} at coordinates ({click_x}, {click_y})")
                instructions.append("2. Follow any prompts or fill in required information")
                instructions.append("3. Complete the action by clicking submit/confirm buttons")
            else:
                instructions.append("1. Review the available interactive elements on the page")
                instructions.append("2. Click on the element that matches your goal")
                instructions.append("3. Follow any prompts or fill in required information")
                instructions.append("4. Complete the action by clicking submit/confirm buttons")
    ...
    def display_instructions(self, instructions: List[str]):
        ...
        if main_instructions:
            ...
            for i, instruction in enumerate(main_instructions, 1):
                ...
                wrapped_instruction = [instruction[i:i+56] for i in range(0, len(instruction), 56)]
        ...
        if safety_reminders:
            ...
            for reminder in safety_reminders:
                # Wrap long reminders
                wrapped_reminder = [reminder[i:i+56] for i in range(0, len(reminder), 56)]
