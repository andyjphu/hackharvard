...
class ReasoningEngine:
    ...
    def _fallback_reasoning(
        self, goal: str, perception: Dict[str, Any], knowledge: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Fallback reasoning when Gemini is not available"""
        print("   🔄 Using rule-based reasoning...")

        elements = perception.get("ui_signals", [])
        system_state = perception.get("system_state", {})
        ...
        if "battery" in goal.lower() or "power" in goal.lower():
            ...
            for element in elements:
                if "low_power" in element.get("id", "").lower():
                    battery_level = (
                        system_state.battery_level
                        if hasattr(system_state, "battery_level")
                        else system_state.get("battery_level", 100)
                    )
                    if battery_level < 50:
                        plan.append(
                            {
        ...
        if "security" in goal.lower():
            ...
            for element in elements:
                if any(
                    security_term in element.get("id", "").lower()
